  ...
  webhookRouter.post("/api/fingerprint", async (req, res) => {
    try {
      ...
      if (!existing) {
        ...
        console.log("Fingerprint stored (new visitor)");
        return res.status(201).json({ success: true, hits: 1 });
      }

      // Document exists → check lastSeenAt for daily hit count
      const lastSeen = existing.lastSeenAt || existing.createdAt;
      const isSameDay = lastSeen >= startOfToday;
